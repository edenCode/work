...
int setupDatagramSocket(UsageEnvironment& env, Port port) 
{
	...
	if (setsockopt(newSocket, SOL_SOCKET, SO_RCVBUF, (char*)&bufsize, sizeof(int)) < 0)
	{
		socketErr(env, "setsockopt(SO_REUSEADDR) error: ");
		closeSocket(newSocket);
		return -1;
	}
	if (setsockopt(newSocket, SOL_SOCKET, SO_SNDBUF, (char*)&bufsize, sizeof(int)) < 0)
	{
		socketErr(env, "setsockopt(SO_REUSEADDR) error: ");
		closeSocket(newSocket);
		return -1;
...
int setupStreamSocket(UsageEnvironment& env,Port port, Boolean makeNonBlocking) 
{
	...
	if (setsockopt(newSocket, SOL_SOCKET, SO_REUSEADDR,(const char*)&reuseFlag, sizeof(int)) < 0) {
			socketErr(env, "setsockopt(SO_REUSEADDR) error: ");
			closeSocket(newSocket);
			return -1;
	}
	...
	if (setsockopt(newSocket, SOL_SOCKET, SO_RCVBUF, (char*)&bufsize, sizeof(int)) < 0){
		socketErr(env, "setsockopt(SO_RCVBUF) error: ");
		closeSocket(newSocket);
		return -1;
	}
	if (setsockopt(newSocket, SOL_SOCKET, SO_SNDBUF, (char*)&bufsize, sizeof(int)) < 0){
		socketErr(env, "setsockopt(SO_SNDBUF) error: ");
		closeSocket(newSocket);
		return -1;
	}
	if (setsockopt(newSocket,IPPROTO_TCP, TCP_NODELAY, (char*)&optval, sizeof(int))  < 0) {
