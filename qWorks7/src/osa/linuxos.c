...
typedef struct os_task 
{
	...
	OSTASK_FXN entry; /* Task entry point */
...
static struct
{
    DL_LIST qHead; /* list of all tasks */
...
{
    DLL_INIT(&gTaskGlobalInfo.qHead);
...
static void registerTask( OSTASK_HANDLE me )
{
	...
	hTask = (OSTASK_HANDLE)DLL_FIRST(&gTaskGlobalInfo.qHead);
	while (hTask!=NULL) {
	...
	if (hTask==NULL) 
		dllAdd(&gTaskGlobalInfo.qHead, (DL_NODE*)me);
...
static void deregisterTask( OSTASK_HANDLE me )
{
	...
	if (hTask!=NULL) 
		dllRemove(&gTaskGlobalInfo.qHead, (DL_NODE*)me);
