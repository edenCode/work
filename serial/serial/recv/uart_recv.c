...
int main(int argc,const char * argv[])
{
	...
	char buffer[sizeof(Header) + psize];
	...
	while(1)
	{
		...
		{
			pHead = (Header *)buffer;
			if(memcmp(pHead->magic,magic,4) == 0)
			{
				//printf(">>>> id %d,size %d,psize %d,counts %d,fileName %s ,MD5 %s \n",pHead->num,pHead->size,pHead->counts,pHead->name,pHead->MD5);
				
				MD5_code((unsigned char *)md5,MD5_SIZE,(unsigned char *)buffer + sizeof(Header),pHead->psize);
				...
				{
					strcpy(pHead->MD5,F);
					printf(" << Failed send >>id %d,size %d,psize %d,counts %d,fileName %s ,MD5 %s \n",pHead->num,pHead->size,pHead->counts,pHead->name,pHead->MD5);
				}
			}
			else
			{
