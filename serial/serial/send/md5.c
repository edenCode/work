...
void md5_encode(unsigned char * output, unsigned int * input, unsigned int len)
{
	...
	{
		output[j] 		= (unsigned int)(input[i] & 0xff);
		output[j + 1] 	= (unsigned int)((input[i] >> 8) & 0xff);
		output[j + 2] 	= (unsigned int)((input[i] >> 16) & 0xff);
		output[j + 3] 	= (unsigned int)((input[i] >> 24) & 0xff);
	}
}
...
void md5_transform(unsigned int * state, const unsigned char * block)
{
...
void md5_fini(unsigned char * digest, MD5_CTX* context)
{
...
{
	MD5_CTX MDContext;
	unsigned char bytTmp[32] = {0};
	unsigned char * pbytOutput = output;
	unsigned int idx;

	md5_init(&MDContext);
	md5_update(&MDContext, input, len);
	md5_fini(bytTmp, &MDContext);
	
	for (idx = 0; idx < 16; ++idx)
	{
		char item[3] = {0};
		sprintf(item, "%02X", bytTmp[idx]);
		memcpy(pbytOutput + idx * 2, item, 2);
	}
}
