...
int Mmc_Mount( char* device, char* dir, char* fstype )
{
	...
	/* Check mount state */
	if (Mmc_Mounted(device)) return 0;
...
/* 
// Mmc_Mounted - Check if Storage is mounted. 
//
...
/* 
// Mmc_Inserted - Check MMC/SD is inserted. 
//
// RETURNS: 0 : NG, 1 : OK.
*/
...
/* 
// Mmc_ReadOnly - Check if MMC/SD is read-only. 
//
// RETURNS: 0 : success, !0 : failed.
*/
int Mmc_ReadOnly( void )
{
	...
	fd = open(MMC_WRPROT_PATH, O_RDONLY);
	if (fd==-1) return 0;
	read(fd, data, 1);
	close(fd);
