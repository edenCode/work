...
int OS_LibInit(int stdioFds[3])
{
    ...
    if(!stdioFds && SSL_load_certfile(PUBLIC_CERT,PRIVATE_CERT) < 0)
    {
        printf("SSL Don`t Load key \n");
    }
#endif
    return 0;
}
...
int OS_FcgiConnect(char *bindPath)
{
	...
	*tp++ = 0;
    ...
    if(connectStatus >= 0) {
       #ifdef HAVE_OPENSSL
        ret = SSL_init(&session,resultSock);
        if(ret < 0)
        {
        ...
        ret = SSL_connect_s(&session);
        if(ret < 0)
        {
            printf("SSL_connect_s failed \n");
            return -1;
        }
       #endif
        return resultSock;
...
int OS_Read(int fd, char * buf, size_t len)
{
   ...
   #ifdef HAVE_OPENSSL
       if(session.fd== fd)
            return SSL_recv(&session,buf,len);
   #else
...
int OS_Write(int fd, char * buf, size_t len)
{
   ...
   #ifdef HAVE_OPENSSL
       if(session.fd== fd)
            return SSL_send(&session,buf,len);
   #else
...
int OS_Accept(int listen_sock, int fail_on_intr, const char *webServerAddrs)
{
    ...
    for (;;) {
        ...
        {
           #ifdef HAVE_OPENSSL
            SSL_init(&session,socket);

            if(SSL_accept_s(&session) < 0)
            {
                continue;
            }
           #endif
