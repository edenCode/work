...
int main ()
{
  ...
  fp = fopen("./GB2312ToUTF8.table","w+");
  fprintf(fp,"GB2312\tUTF8\tUNICODE\tcharset \n");
...
void printOutput(GB2312 gb, UTF8 utf, UNICODE uni)
{
  ...
  int i;
  printf("GB2312 %d,utf-8 : %d,unicode : %d \n",sizeof(gb),len,sizeof(uni));
  fprintf(fp,"%2x%2x\t", gb[0], gb[1]);
  ...
  fprintf(fp,"%u\t", (unsigned int) uni);

  if (fwrite(utf,len,1,fp) != 1)
